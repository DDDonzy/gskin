...
class BufferManager:
    ...
    __slots__ = ("ctypes", "format_char", "ptr", "shape", "view")

    _CTYPES_MAP = {  # noqa: RUF012
        "d": ctypes.c_double,
        "f": ctypes.c_float,
        "i": ctypes.c_int32,
        "I": ctypes.c_uint32,
    ...
    @staticmethod
    def auto(data, format_char: str = "f", shape: tuple | None = None):
        """
        [万能路由函数]
        根据输入对象类型自动选择最优的构建路径
    ...
    @staticmethod
    def from_list(data_list: list, format_char: str = "f", shape: tuple | None = None):
    ...
    @staticmethod
    def from_buffer(data, format_char: str, shape: tuple | None = None):
        """
    ...
    @staticmethod
    def from_ctypes(ctypes_array, format_char: str = "i", shape: tuple | None = None):
        """
        [终极 0 拷贝接收器]
    ...
    def slice(self, start: int, end: int | None = None) -> BufferManager:
        """
        [极速零拷贝切片]
